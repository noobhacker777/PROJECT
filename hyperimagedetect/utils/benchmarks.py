...
from hyperimagedetect.cfg import TASK2DATA, TASK2METRIC
from hyperimagedetect.engine.exporter import export_formats
from hyperimagedetect.utils import ARM64, ASSETS, ASSETS_URL, IS_JETSON, LINUX, LOGGER, MACOS, TQDM, WEIGHTS_DIR, YAML
from hyperimagedetect.utils.checks import IS_PYTHON_3_13, check_imgsz, check_requirements, check_holo, is_rockchip
from hyperimagedetect.utils.files import file_size
from hyperimagedetect.utils.torch_utils import get_cpu_info, select_device

def benchmark(
    model=WEIGHTS_DIR / "holo11n.pt",
    data=None,
    imgsz=160,
    half=False,
    int8=False,
    ...
    for name, format, suffix, cpu, gpu, _ in zip(*export_formats().values()):
        ...
        try:
            ...
            if format == "imx":
                ...
                assert "C2f" in model.__str__(), "IMX only supported for holo11n"
            if format == "rknn":
            ...
            if format == "-":
                ...
                exported_model = HOLO(filename, task=model.task)
...
class RF100Benchmark:
    ...
    def parse_dataset(self, ds_link_txt: str = "datasets_links.txt"):
        ...
        os.chdir("rf-100")
        os.mkdir("hyperimagedetect-benchmarks")
