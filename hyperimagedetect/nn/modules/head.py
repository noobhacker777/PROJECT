...
from hyperimagedetect.utils.tal import dist2bbox, dist2rbox, make_anchors
from hyperimagedetect.utils.torch_utils import TORCH_1_11, fuse_conv_and_bn, smart_inference_mode
...
__all__ = "OBB", "Classify", "Detect", "Pose", "RTDETRDecoder", "Segment", "HOLOEDetect", "HOLOESegment", "v10Detect"
...
class HOLOESegment(HOLOEDetect):
    ...
    def forward(self, x: list[torch.Tensor], text: torch.Tensor) -> tuple | torch.Tensor:
        ...
        if not has_lrpc:
            x = HOLOEDetect.forward(self, x, text)
        else:
            x, mask = HOLOEDetect.forward(self, x, text, return_mask=True)
...
class RTDETRDecoder(nn.Module):
    ...
    def forward(self, x: list[torch.Tensor], batch: dict | None = None) -> tuple | torch.Tensor:
        from hyperimagedetect.models.utils.ops import get_cdn_group
