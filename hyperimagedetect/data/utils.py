...
from hyperimagedetect.utils.checks import check_file, check_font, is_ascii
from hyperimagedetect.utils.ops import segments2boxes
...
HELP_URL = "See https://docs.hyperimagedetect.com/datasets for dataset formatting guidance."
...
def check_file_speeds(
    ...
    if avg_ping < threshold_ms or avg_speed < threshold_mb:
        ...
        LOGGER.warning(
            ...
            f"See https://docs.hyperimagedetect.com/guides/model-training-tips/"
        )
...
def visualize_image_annotations(image_path: str, txt_path: str, label_map: dict[int, str]):
    ...
    from hyperimagedetect.utils.plotting import colors
...
def check_det_dataset(dataset: str, autodownload: bool = True) -> dict[str, Any]:
    ...
    for k in "train", "val":
        if k not in data:
            ...
            LOGGER.warning("renaming data YAML 'validation' key to 'val' to match HOLO format.")
    ...
    if val:
        ...
        if not all(x.exists() for x in val):
            LOGGER.info("")
            m = f"Dataset images not found, missing path '{next(x for x in val if not x.exists())}'"
...
def check_cls_dataset(dataset: str | Path, split: str = "") -> dict[str, Any]:
    ...
    if not data_dir.is_dir():
        if data_dir.suffix != "":
            raise ValueError(
                ...
                "See https://docs.hyperimagedetect.com/datasets/classify/"
            )
        LOGGER.info("")
    ...
    if not train_set.is_dir():
        ...
        if image_files := list(data_dir.rglob("*.jpg")) + list(data_dir.rglob("*.png")):
            from hyperimagedetect.data.split import split_classify_dataset
    ...
    def get_json(self, save: bool = False, verbose: bool = False) -> dict:
        ...
        for split in "train", "val", "test":
            ...
            if self.task == "classify":
                ...
                dataset = HOLODataset(img_path=self.data[split], data=self.data, task=self.task)
                x = np.array(
                    [
    ...
    def process_images(self) -> Path:
        ...
        for split in "train", "val", "test":
            if self.data.get(split) is None:
                continue
            dataset = HOLODataset(img_path=self.data[split], data=self.data)
